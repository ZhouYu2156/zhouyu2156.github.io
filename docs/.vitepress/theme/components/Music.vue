...
<template>
  <section class="jkt-music" aria-labelledby="treasured-music-heading">
    ...
    <ul v-else-if="loaded" class="jkt-music__grid">
      <li v-for="(item, index) in items" :key="item.id" class="group jkt-music__card"
        :class="{ 'jkt-music__card--current': index === currentIndex }">
        <div class="jkt-music__cover-wrap">
          <img class="jkt-music__cover" :src="item.cover" :alt="`${item.song_name} 封面`" loading="lazy"
            decoding="async" />
          <div class="jkt-music__cover-overlay" aria-hidden="true" />
          <button type="button" class="jkt-music__play-btn"
            :aria-label="index === currentIndex && isPlaying ? `暂停 ${item.song_name}` : `播放 ${item.song_name}`"
            @click="onPlay(index, $event)">
            <svg v-if="index === currentIndex && isPlaying" viewBox="0 0 24 24" fill="currentColor"
              class="jkt-music__play-icon" aria-hidden="true">
              <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" />
            </svg>
            <svg v-else viewBox="0 0 24 24" fill="currentColor" class="jkt-music__play-icon" aria-hidden="true">
...
.jkt-music__error {
  @apply mt-10 text-center text-sm;
  color: var(--vp-c-danger-1, #b91c1c);
}

html.dark .jkt-music__error {
...
.jkt-music__card {
  @apply flex translate-y-0 flex-col overflow-hidden rounded-xl border border-(--vp-c-divider) bg-(--vp-c-bg) shadow-[0_1px_3px_rgba(15_23_42_0.06)] transform-gpu backface-hidden dark:shadow-[0_1px_3px_rgba(0_0_0_0.45)];
...
.jkt-music__card:hover {
  @apply -translate-y-px border-[color-mix(in_srgb_var(--vp-c-brand-1)_40%_var(--vp-c-divider))] shadow-[0_10px_28px_rgba(15_23_42_0.12)] dark:shadow-[0_12px_32px_rgba(0_0_0_0.55)];
}
